...
import { seriesId } from '../config/Helpers.js';
	...
	useEffect(
		...
		() => {
			...
			async function fetchData() {
				try {
					const { data: show } = await axios.get(seriesId(props.match.params.id));
					console.log(show)
					setShowData(show);
					setLoading(false);
				} catch (e) {
	...
	const buildSingleData = (show) => {
		...
		return (
			<Card className={classes.card} variant='outlined'>
				<CardHeader className={classes.titleHead} title={show.title } />
				<CardMedia
				...
				<dl>
							...
							<dl>
								<dt className='title'>Characters which appear in this series :</dt>
								{show && show.characters.items && show.characters.items.length >= 1 ? (
								
										show.characters.items.map((genre) => {
											if (show.characters.items > 1) return <dd key={genre.name}>{genre.name},</dd>; 
							...
							<dl>
								<dt className='title'>List containing comics in this series :</dt>
